...
static int zlog_rotater_add_archive_files(zlog_rotater_t * a_rotater)
{
	...
	for (; pathc-- > 0; pathv++) {
		...
		rc = zc_arraylist_sortadd(a_rotater->files,(zc_arraylist_cmp_fn)zlog_file_cmp, a_file);
		if (rc) {
		...
		int *reopen_fd, int reopen_flags, unsigned int reopen_perms)
{
	...
	if (stat(base_path, &info)) {
		rc = -1;
		zc_error("fstat [%s] fail, errno[%d]", base_path, errno);
		goto exit;
	}

	if (info.st_size + msg_len <= archive_max_size) {
		/* file not so big,
		 * mayf alread rotate by oth process or thread,
		 * return */
		rc = 0;
		goto exit;
	}
