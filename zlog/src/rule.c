...
static int zlog_rule_output_static_file_rotate(zlog_rule_t * a_rule, zlog_thread_t * a_thread)
{
	...
	//if (a_rule->fsync_period && ++a_rule->fsync_count >= a_rule->fsync_period) {
		//a_rule->fsync_count = 0;
		if (FlushFileBuffers(a_rule->static_fd)) {
			zc_error("fsync[%d] fail, errno[%d]", a_rule->static_fd, errno);
		}
	...
	if (zlog_rotater_rotate(zlog_env_conf->rotater,a_rule->file_path, len,	
		zlog_rule_gen_archive_path(a_rule, a_thread),a_rule->archive_max_size, 
		a_rule->archive_max_count,&(a_rule->static_fd), 
		...
		size_t fsync_period)
{
	...
	case '"' :
		...
		if (a_rule->dynamic_specs) {
			...
			a_rule->static_fd = zlogopen(a_rule->file_path, O_WRONLY | O_APPEND | O_CREAT | a_rule->file_open_flags,a_rule->file_perms);
